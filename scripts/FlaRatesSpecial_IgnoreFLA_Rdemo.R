#calculate rates ignoring FLA bin for those manually identified in igfla

#the most updated rates file you want to update rates for
master.rates <- "FlaRatesZeroCorrected_Rdemo.csv"
#the relative path to the reference file that defines which incubations you want to ignore the fla bin for
igfla.ref <- "reference-files/IgnoreFlaBin_shelf1234.txt"
#name folder in your wd in which have slant corrected data want to calc rates with
CsvDir <- "slant-corrected-csv/" 
#relative path to your hydrolysis cuts info
cuts.info <- "reference-files/HydrolysisCutsInfo.csv"
#relative path to your timesheet and std ref file
timestd.ref <- "reference-files/FlaTimepointsStdRefs_EN556stn1234.csv"
#relative path to your std bins files (as listed in your timestd.ref sheet)
std.bins <- "reference-files/stdbins/"
#outputMaster is the main sheet where you will record all your rates info
outputMaster <- "FlaRatesFinal_Rdemo.csv"
...
#read in most updated rates file want to update rates for 
master.bulk <- read.csv(master.rates, row.names=1)
...
igfla <- read.table(igfla.ref,sep=",",colClasses="character")
#Read in info about mw of substrates and the # of cuts needed to get to a certain std size bin
cuts <- read.csv(cuts.info, header=TRUE, row.names=1)
#Read in table of sampling times and the Elapsed Time since t0
FLAElapsed <- read.csv(timestd.ref, header=TRUE, row.names=1)

#looks for any files in your CsvDir that contain the strings in your igfla ref file; these are the incubations for which new rates will be calculated
...
for (j in 1:length(RatesMasterList)) {
    ...
    StdBins <- read.csv(paste(std.bins,stdsID,".csv",sep=""),header=TRUE)
