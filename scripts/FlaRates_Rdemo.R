...
substrates <- c("ara","chon","fuc","lam","pul","xyl") #if you change these, you'll have to change your "HydrolysisCutsInfo.csv" column names as well
...
cuts <- read.csv("reference-files/HydrolysisCutsInfo.csv", header=TRUE, row.names=1)
#Read in table of sampling times and the Elapsed Time since t0
FLAElapsed <- read.csv("reference-files/FlaTimepointsStdRefs_EN556stn1234.csv", header=TRUE, row.names=1)
#the folder where your standard bin files are 
StdBinPath <- "reference-files/stdbins/"
#FLAMaster is the main sheet where you will record all your rates info
outputMaster <- "FlaRates_Rdemo.csv"
