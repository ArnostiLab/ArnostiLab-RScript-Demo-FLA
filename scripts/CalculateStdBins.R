...
csvDir <- "stds/stds-gpc2-071515/slant-corrected-csv"   
#filename want output std bin info file to be, in csv format
outputPath <- "stds/stds-gpc2-071515/stdbins-gpc2-071515.csv"
#filename want std bin visualization to be, in png format
pngPath <- "stds/stds-gpc2-071515/stdbins-gpc2-071515.png"
...
print(min(a)-72) 
print("bin150 time")
print(StdData$time[min(a)-60]) 

StdBins <- data.frame(bin=c("150kD","10kD","4kD","Gal","FLA"),read.number=c(min(a)-60), time=c(StdData$time[min(a)-60]))
...
for (j in 3:length(StdData)) {
  ...
  #if there is more than one int.point, for example if you have low intensity chromatograms and the noisy baseline causes the not-real-intersection bouncy baselines to intersect, choose the intersection where the fluorescence is maximum (where your real signal is)
  if (length(int.point)>1) {
      int.point <- int.point[which.max(StdData[,j][int.point])]
  }
