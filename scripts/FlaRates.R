...
#e.g. stn7-d0-bulk-lam-x-t3 or stn4-d5-bulk-chn-1-t0

#define path where your main directory is
path <- getwd()
...
substrates <- c("ara","chn","fuc","lam","pul","xyl") #if you change these, you'll have to change your "HydrolysisCutsInfo.csv" column names as well
#incubation volume for bulk incs in EN584 is 15mL=0.015L
IncVolume <- 0.015
#a data frame of the monomer-equivalent substrate concentration in incubations for each substrate, in uM
MonEquivSubConc <- data.frame(ara=3.5,chn=3.5,fuc=5,lam=3.5,pul=3.5,xyl=3.5)
...
FLAElapsed <- read.csv("TN362_Bulk_Rates_SampleSheet.csv", header=TRUE, row.names=1)
#the folder where your standard bin files are 
StdBinPath <- "std-bins/"
#FLAMaster is the main sheet where you will record all your rates info
outputMaster <- "FlaRates_TN362_071922.csv"
...
CsvNameListBulk <- CsvNameList[grep("stn([0-9a-z]+)-d([0-9])-bulk-([a-z]+)-([0-9a-z])-t([0-9]).csv",CsvNameList)]
...
print("reading in and organizing GPC files...")
...
for (i in 1:length(CsvNameListBulk)) {
    ...
    PartialName <- sub("stn([0-9a-z]+)-d([0-9])-bulk-([a-z]+)-([0-9a-z])-t([0-9]).csv","stn\\1-d\\2-bulk-\\3",CsvName)
    IncName <- sub("stn([0-9a-z]+)-d([0-9])-bulk-([a-z]+)-([0-9a-z])-t([0-9]).csv","stn\\1-d\\2-bulk-\\3-\\4",CsvName)
    FullName <- sub("stn([0-9a-z]+)-d([0-9])-bulk-([a-z]+)-([0-9a-z])-t([0-9]).csv","stn\\1-d\\2-bulk-\\3-\\4-t\\5",CsvName)
    TimeName <- sub("stn([0-9a-z]+)-d([0-9])-bulk-([a-z]+)-([0-9a-z])-t([0-9]).csv","stn\\1-d\\2-bulk-\\3-t\\5",CsvName)
...
for (j in 1:length(RatesMasterList)) {
    ...
    StdBins <- read.csv(paste(StdBinPath,stdsID,".csv",sep=""),header=TRUE)
    ...
    #remove columns from PercentTotal by IncName (e.g. all columns that start with stn10-d2-chn-1), calculate ChangeZero, insert into correct column in ChangeZero
    ...
    for (v in 1:length(colnames(hydrolysisEvents))) {
        timeElapsed <- FLAElapsed[colnames(hydrolysisEvents)[v],"FLAElapsed"]
        volumeL <- IncVolume
    ...
    for (x in 1:length(TimeList[[j]])) {
        ...
        names(R2) <- gsub(pattern="stn([0-9a-z]+)-d([0-9])-bulk-([a-z]+)-([0-9a-z])-t([0-9])","rate.\\4.nM.hr",names(R2))
        names(kcR2) <- gsub(pattern="stn([0-9a-z]+)-d([0-9])-bulk-([a-z]+)-([0-9a-z])-t([0-9])","kcrate.\\4.nM.hr",names(kcR2))
